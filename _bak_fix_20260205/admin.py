# inventory/admin.py (production-ready, deduped)
...
def _get_scope(request: HttpRequest) -> Dict[str, Any]:
    ...
    aimag_id = getattr(prof, "aimag_id", None) or getattr(prof, "aimag_ref_id", None)
    sum_id = (
...
def _loc_label(o: Location) -> str:
    return (
        getattr(o, "name", None)
        or getattr(o, "name_mn", None)
        or getattr(o, "code", None)
        or getattr(o, "wmo_index", None)
        or f"Location#{getattr(o, 'pk', '')}"
    )
...
class SumDuuregByAimagFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        # УБ дүүрэг зөвхөн УБ дээр харагдах
        try:
            ub_id = get_ub_aimag_id()
...
class LocationTypeFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        try:
            field = Location._meta.get_field("location_type")
...
def generate_qr(modeladmin, request: HttpRequest, queryset: QuerySet):
    ...
    for d in queryset:
        ...
        # URL build (inventory namespace байх ёстой: inventory:qr_device_lookup)
        try:
...
class SumDuuregAdmin(admin.ModelAdmin):
    list_display = ("aimag", "name", "is_ub_district")
...
class LocationAdmin(admin.ModelAdmin):
    ...
    class Media:
        js = (
            ...
            "inventory/js/admin/admin_location_dropdown.js",
        )
...
class DeviceAdmin(admin.ModelAdmin):
    ...
    def passport_view(self, request: HttpRequest, object_id: int):
        ...
        return FileResponse(
            ...
            as_attachment=True,
            filename=f"passport_{device.pk}.pdf",
    ...
    def location_options_view(self, request: HttpRequest):
        ...
        results = [{"id": l.id, "text": _loc_label(l)} for l in qs]
        return JsonResponse({"results": results})
...
class MaintenanceServiceAdmin(admin.ModelAdmin):
    list_display = ("date", "device", "reason", "workflow_status", "performer_type", "performer_engineer_name", "performer_org_name")
    ...
    def get_queryset(self, request: HttpRequest) -> QuerySet:
        qs = super().get_queryset(request).select_related("device", "device__location", "device__location__aimag_ref")
...
class ControlAdjustmentAdmin(admin.ModelAdmin):
    ...
    list_filter = ("workflow_status", "performer_type")
    search_fields = ("device__serial_number", "device__inventory_code", "result", "note")
    ordering = ("-date", "-id")
