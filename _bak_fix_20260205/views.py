...
# ---------------------------------------------------------------------
@staff_member_required(login_url="/django-admin/login/")
def location_map(request: HttpRequest) -> HttpResponse:
    """
    Байршлын газрын зураг (multi).
    Query params (optional): aimag, sum, district, location_type, device_kind, device_status
    """
    def _p(*keys: str) -> str:
        for k in keys:
    ...
    aimag_id = _int(_p("aimag", "aimag_ref__id__exact", "aimag_ref_id"))
    sum_id = _int(_p("sum", "sum_ref__id__exact", "sum_ref_id", "sumduureg"))
    district = _p("district", "district_name", "district_name__exact")
    location_type = _p("location_type", "location_type__exact", "loc_type").upper()
    device_kind = _p("device_kind", "kind").upper()
    device_status = _p("device_status", "status")
    ...
    # Device filters (related_name="devices")
    if device_kind:
        qs = qs.filter(devices__kind__iexact=device_kind)
    if device_status:
        qs = qs.filter(devices__status=device_status)
        ...
        items.append(
            {
                ...
                "last_control_date": str(r.get("last_control_date") or ""),
            }
        )
...
@staff_member_required(login_url="/django-admin/login/")
def location_map_one(request: HttpRequest, location_id: int) -> HttpResponse:
    """Нэг байршил газрын зураг."""
    ...
    return render(
        ...
        {"location_json": json.dumps(item, ensure_ascii=False, cls=DjangoJSONEncoder)},
    )
...
@staff_member_required(login_url="/django-admin/login/")
def qr_device_lookup(request: HttpRequest, token) -> HttpResponse:
    ...
    # custom admin site ашиглаж байгаа үед:
    try:
        url = reverse("inventory_admin:inventory_device_change", args=[device.pk])
    except Exception:
        url = reverse("admin:inventory_device_change", args=[device.pk])
    return redirect(url, permanent=False)


def qr_device_public_view(request: HttpRequest, token) -> HttpResponse:
    ...
    # PDF татах линк (inventory namespace байж магадгүй)
    pdf_url = "#"
    for name in ("inventory:qr_device_public_passport_pdf", "qr_device_public_passport_pdf"):
        try:
            pdf_url = reverse(name, args=[token])
            break
        except Exception:
            continue
...
def qr_device_public_passport_pdf(request: HttpRequest, token) -> HttpResponse:
...
# ---------------------------------------------------------------------
# 4) Compatibility shortcuts (for Jazzmin links)
# ---------------------------------------------------------------------
@staff_member_required(login_url="/django-admin/login/")
def reports_hub_redirect(request: HttpRequest) -> HttpResponse:
    """Template/Jazzmin дээр reverse('reports-hub') дуудагдвал admin Reports руу чиглүүлнэ."""
    return redirect("/django-admin/reports/", permanent=False)
