...
def _scope_location_qs(request: HttpRequest) -> QuerySet[Location]:
    return _scope_qs(request, Location.objects.all(), aimag_field="aimag_ref")
...
def generate_device_passport_pdf_bytes(device: Device) -> bytes:
    ...
    rows = [
        ...
        [
            "Ашиглалтад орсон",
            str(getattr(device, "commissioned_date", None) or getattr(device, "installation_date", None) or "---"),
        ],
    ]
...
class LocationTypeFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [
            ("WEATHER", "Цаг уур"), ("AWS", "AWS"), ("RADAR", "Радар"),
            ("HYDRO", "Ус судлал"), ("AEROLOGY", "Аэрологи"), ("AGRO", "ХАА"),
            ("ETALON", "Эталон"), ("OTHER", "Бусад"),
        ]
...
def generate_qr(modeladmin, request: HttpRequest, queryset: QuerySet):
    ...
    for d in queryset:
        ...
        route_name = "inventory:qr_device_public" if public else "inventory:qr_device_lookup"
...
class DeviceAdmin(admin.ModelAdmin):
    ...
    def passport_view(self, request, object_id):
        ...
        return HttpResponse(f"Device Passport placeholder: {object_id}")
...
class InventoryAdminSite(AdminSite):
    ...
    def index(self, request, extra_context=None):
        return redirect(reverse(f"{self.name}:dashboard_general"), permanent=False)
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path(
                "dashboard/general/",
                self.admin_view(general_dashboard_view),
            ...
            path(
                "api/reports/charts/",
                self.admin_view(rhc.reports_chart_json),
                name="reports-chart-json",
            ),
            path(
                "api/reports/sums/",
                self.admin_view(rhc.reports_sums_by_aimag),
                name="reports-sums-by-aimag",
            ),
            path(
                "reports/export/devices.csv",
                self.admin_view(rhc.reports_export_devices_csv),
                name="reports-export-devices-csv",
            ),
            path(
                "reports/export/locations.csv",
                self.admin_view(rhc.reports_export_locations_csv),
                name="reports-export-locations-csv",
            ),
            path(
                "reports/export/maintenance.csv",
                self.admin_view(rhc.reports_export_maintenance_csv),
                name="reports-export-maintenance-csv",
            ),
            path(
                "reports/export/movements.csv",
                self.admin_view(rhc.reports_export_movements_csv),
                name="reports-export-movements-csv",
            ),

            # Workflow
            path(
                "inventory/workflow/",
                self.admin_view(wf.workflow_pending_dashboard),
                name="workflow-pending-dashboard",
            ),
            path(
            ...
            path(
                ...
                self.admin_view(wf.workflow_review_action),
                name="workflow-review-queue",
            ),
            path(
                "inventory/workflow/audit/",
                self.admin_view(wf.workflow_audit_log),
                name="workflow-audit-history",
            ),

            # Legacy / custom menu support
            path(
                "data-entry/",
                self.admin_view(lambda request: redirect("/django-admin/inventory/device/")),
                name="admin-data-entry",
            ),
            path(
                "dashboard/table/",
                self.admin_view(lambda request: redirect("/django-admin/reports/")),
                name="dashboard-table",
            ),
            path(
                "dashboard/graph/",
                self.admin_view(lambda request: redirect("/django-admin/dashboard/general/")),
                name="dashboard-graph",
            ),
            path(
                "map/",
                self.admin_view(lambda request: redirect("/django-admin/inventory/location/map/")),
                name="dashboard-map",
            ),
        ]
        return custom_urls + urls
